...
- `{{CONFIG_DIR}}/canonical/content-intelligence.md` (performance baselines + content language to test)
- `{{DATA_DIR}}/memory/evergreen-ideas.md` (check Available section first for reusable angles)
- `{{DATA_DIR}}/memory/recent-changes.md` (canonical changes this week = potential content angles)
...
3. **{{TARGET_PERSONA}} pain echo**: riff on one of the validated buyer pains without naming your product. Read `{{CONFIG_DIR}}/canonical/talk-tracks.md` for validated pain language.
...
6. **Canonical wedge**: from talk tracks in digest. Rotate, don't repeat last 2 weeks. Read `{{CONFIG_DIR}}/canonical/talk-tracks.md` for wedge themes.
7. **Counter-narrative**: argue opposite of a popular take from linkedin-posts.json.

## LinkedIn TL post
Start with scar, number, or sharp claim. NEVER question. NEVER "I". Max 200 words. Frame around validated buyer pains from `{{CONFIG_DIR}}/canonical/talk-tracks.md`. No product pitch. End with implicit question or sharp closer. **Loss framing is valid for awareness content (LinkedIn TL).** Gain framing is for buyer conversion only.
