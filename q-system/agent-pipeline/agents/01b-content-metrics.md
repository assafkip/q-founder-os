...
## Writes
- `{state_dir}/bus/{date}/content-metrics.json`
...
2. For each of the last 10 posts (or however many are visible):
   ...
   - Classify post_type based on content: "signals" (threat intel), "tl" (thought leadership), "hot_take", "bts" (behind the scenes), "product" (product promo)
3. If Chrome fails to load analytics after 2 attempts, write `{"date": "{{DATE}}", "metrics": [], "error": "chrome_failed"}` and exit.
4. Write results to `{state_dir}/bus/{date}/content-metrics.json`:
  ...
  "metrics": [
    {
      ...
      "post_type": "signals|tl|hot_take|bts|product",
      "impressions": 1234,
      "likes": 45,
      "comments": 12,
      "reposts": 3,
