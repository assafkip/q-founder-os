...
function main() {
  ...
  const result = {
    ...
    instructions: [
      ...
      "",
      "2. Or use the ai-multimodal skill (external dependency, not in this plugin):",
      `   python .claude/skills/ai-multimodal/scripts/gemini_batch_process.py \\`,  // external skill path
      `     --files "${resolvedPath}" \\`,
      `     --task analyze \\`,
