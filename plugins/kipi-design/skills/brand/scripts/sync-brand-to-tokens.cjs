...
// Cross-skill reference: generate-tokens.cjs lives in the design skill (same plugin)
const GENERATE_TOKENS_SCRIPT = path.join(__dirname, '..', '..', 'design', 'scripts', 'generate-tokens.cjs');
...
function main() {
  ...
  // Regenerate CSS
  const generateScript = path.resolve(GENERATE_TOKENS_SCRIPT);
