...
**Brand:**
```bash
node ${CLAUDE_SKILL_DIR}/../brand/scripts/inject-brand-context.cjs
node ${CLAUDE_SKILL_DIR}/../brand/scripts/validate-asset.cjs <path>
```

**Tokens:**
```bash
node ${CLAUDE_SKILL_DIR}/scripts/generate-tokens.cjs -c tokens.json
node ${CLAUDE_SKILL_DIR}/scripts/validate-tokens.cjs -d src/
```
